...
def save_answers(request, course_id, que_id):
    if request.method=='POST':
        ...
        if SaveExam.objects.filter(student=student, course=course, quetion=questions, answer=answer).exists():    
            print("update")
            obj = SaveExam.objects.filter(student=student, course=course, quetion=questions).update(answer=answer)
            obj.save()
            if obj:    
